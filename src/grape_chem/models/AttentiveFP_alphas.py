...
class AttentiveFP_alphas(torch.nn.Module):
    r""" A modification of the PyG implementation of AFP to return one of the
    intermediate variables involved in its computation of attention.
    -- Original Doctrsing:
    ...
    def forward(self, x: Tensor, edge_index: Tensor, edge_attr: Tensor,
        ...
        gconv_out = self.gate_conv(x, edge_index, edge_attr)
        ...
        for t in range(self.num_timesteps):
            mol_conv_out, alphas = self.mol_conv((x, out), edge_index, return_attention_weights=True)
            h = F.elu_(mol_conv_out)
            h = F.dropout(h, p=self.dropout, training=self.training)
            out = self.mol_gru(h, out).relu_()

        # Predictor:
        out = F.dropout(out, p=self.dropout, training=self.training)
        return self.lin2(out), alphas
