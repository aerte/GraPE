...
class DMPNNNet(nn.Module):
    def __init__(self, net_params):
        ...
        self.linear_prediction = nn.Sequential(
                nn.Linear(net_params['hidden_dim'] + net_params['global_dim'], int(net_params['hidden_dim']/2), bias=True),
                nn.ReLU(),
    ...
    def forward(self, graph, node, edge, global_feats, get_descriptors=False):
        node = node.float()
        edge = edge.float()
        ...
        concat_feats = torch.cat([graph_feats, global_feats])
        output = self.linear_prediction(concat_feats)
        if get_descriptors:
