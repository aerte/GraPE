...
from grape_chem.utils import (
    DataSet, EarlyStopping, train_epoch_jittable, val_epoch_jittable,
...
def load_model(config, device, frag_dim):
    """Load a single GroupGAT_jittable model based on the config."""
    net_params = {
        "device": device,
        "num_atom_type": 44,
        "num_bond_type": 12,
        ...
        "node_in_dim": 44,
        "edge_in_dim": 12,
        "num_global_feats": 1 if config.get("use_global_features", False) else 0,
        "hidden_dim": int(config["hidden_dim"]),
        "mlp_out_hidden": return_hidden_layers(int(config["MLP_layers"])),
    ...
    return GroupGAT_jittable.GCGAT_v4pro_jit(net_params).to(device)
...
def trainable(config, device, fragmentation, data_config):
    ...
    early_Stopper = EarlyStopping(patience=30, model_name='GroupGAT_jittable', skip_save=True)
    ...
    iterations = config.get('max_epochs', 1000)  # Allow max_epochs to be set via config
    start_epoch = 0
...
if __name__ == '__main__':
    ...
    config_space.add([
        ...
        CS.UniformIntegerHyperparameter("num_layers_atom", lower=1, upper=3),
        CS.UniformIntegerHyperparameter("num_layers_mol", lower=1, upper=4),
        CS.UniformIntegerHyperparameter("MLP_layers", lower=1, upper=3),
        CS.UniformIntegerHyperparameter("num_heads", lower=1, upper=4),
        # L1 parameters
        CS.UniformIntegerHyperparameter("L1_layers_atom", lower=1, upper=3),
        CS.UniformIntegerHyperparameter("L1_layers_mol", lower=1, upper=4),
        ...
        # L2 parameters
        CS.UniformIntegerHyperparameter("L2_layers_atom", lower=1, upper=3),
        CS.UniformIntegerHyperparameter("L2_layers_mol", lower=1, upper=4),
        ...
        # L3 parameters
        CS.UniformIntegerHyperparameter("L3_layers_atom", lower=1, upper=3),
        CS.UniformIntegerHyperparameter("L3_layers_mol", lower=1, upper=4),
    ...
    reporter = tune.CLIReporter(
        parameter_columns=[
            "initial_lr", "weight_decay", "lr_reduction_factor", "dropout",
            "final_dropout", "hidden_dim", "num_layers_atom", "num_layers_mol",
            "MLP_layers", "num_heads", "L1_layers_atom", "L1_layers_mol",
            "L1_dropout", "L1_hidden_dim", "L2_layers_atom", "L2_layers_mol",
            "L2_dropout", "L2_hidden_dim", "L3_layers_atom", "L3_layers_mol",
            "L3_dropout", "L3_hidden_dim", "use_global_features", "batch_size",
            "max_epochs", "checkpoint_freq"
        ],
    ...
    tuner = Tuner(
        ...
        run_config=RunConfig(
            name="groupgat_jittable_icp_hpo",
            stop={"epoch": 1000},
    ...
    file_name = os.path.join(directory, f"best_hyperparameters_GroupGAT_jittable_ICP.json")
    os.makedirs(directory, exist_ok=True)
    with open(file_name, "w") as file:
        json.dump(results_to_save, file, indent=4)

    print(f"Best hyperparameters saved to {file_name}")
