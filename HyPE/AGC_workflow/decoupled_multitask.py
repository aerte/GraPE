...
from grape_chem.models import AGC
...
fragmentation = JT_SubGraph(scheme=fragmentation_scheme, save_file_path="env/AGC_multitask_frag_save")
...
net_params = {
    ...
    # Additional parameters specific to AGC
    "L1_layers_atom": 4,
    "L1_layers_mol": 1,
    "L1_dropout": 0.142,
    "L2_layers_atom": 2,
...
# Initialize the AGC model with output dimension equal to number of targets
...
model_filename = 'agc_latest_multitask.pth'
...
pred = test_model(model=model, test_data_loader=test, device=device, batch_size=batch_size)
...
train_preds = test_model(model=model, test_data_loader=train, device=device, batch_size=batch_size)
val_preds = test_model(model=model, test_data_loader=val, device=device, batch_size=batch_size)
...
test_metrics_per_property, test_overall_metrics = compute_metrics(test_preds_rescaled, test_targets_rescaled, dataset_name='Test')
...
def create_parity_plot(
    ...
    for i, prop in enumerate(target_columns):
        ...
        plt.title(f'Parity Plot for Property {prop} in Decoupled Multitask (AGC)')
        plt.legend(handles=[
...
####### Generating Predictions and Saving #########
def generate_predictions_and_save(
    ...
    all_preds, _ = test_model_with_parity(
        model=model,
...
generate_predictions_and_save(
    ...
    batch_size=batch_size,
    device=device,
    output_filename='predictions_agc_multitask.csv'
)
